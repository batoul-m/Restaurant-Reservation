...
namespace RestaurantReservation.Db
{
    public class RestaurantReservationDbContext : DbContext{
        ...
        {
            var totalRevenue = this.Database.ExecuteSqlRawAsync("SELECT dbo.CalculateTotalRevenue({0})", restaurantId);
            return Convert.ToDecimal(totalRevenue);
            
        }
        public async Task<List<Customers>> FindCustomersWithLargeParties(int partySize)
        {
            var customers = await this.Customers.FromSqlRaw("EXEC FindCustomersWithLargeParties @PartySize = {0}",partySize).ToListAsync();
            return customers;
        }
    }
}